...
每局从基础攻击开始。第 1 轮结束时选择 Pistol、SMG 或 Shotgun；之后击败元素敌人并拾取 Element Core。元素不会自动改变武器：玩家决定何时按 `E` 进化，因此可以在立即进化与保留元素、等待特殊融合之间取舍。
...
Weapon 决定射击节奏与弹道参数；Evolution 决定元素攻击形态。两者相互独立。第 6 轮起，商店会按当前 Weapon × Tier 2 流派提供可叠加的联动强化，但不会给 Tier 2 强行添加第二种攻击形态。
...
- **Round 10 / 12 / 14 / 16 / 18 / 20**：Boss 生存战。坚持 60 秒即可通关该轮；击败 Boss 是可选挑战，成功会获得一项免费的稀有 Buff。
