...
- **Tier 2**（已完成）：45 个有方向或双元素的融合，每个保留一个清晰的主攻击行为，例如 Orbit、Zone、Burst、Beam、Wave 或 Status。（完整的随机可能性）
- **Tier 3**（部分已完成）：45 个命定特殊进化。Tier 3 保留 Tier 2 的主行为，并追加由主行为触发的次级行为，形成真正的复合攻击。（根据完整的每种Tier2可能性暂时做一种进化路径）
- **Tier 4**（部分已完成）：45 个终极特殊进化。Tier 4 保留 Tier 3 的主行为，并加强复合攻击能力，增强至自动追踪等高阶攻击能力。（根据完整的每种Tier3可能性暂时做一种进化路径）


例如：第五轮之前取得 暗 + 风 可以形成 **魂**，让子弹具有攻击敌人后自动向附近敌人散射的特性；
在第 9 轮取得土元素后，可进化为 **魂：傀儡**，增加一个引导自动散射的傀儡。
在第14轮取得光元素后，进化为 **魂：灵魂军团**，所有子弹具有大范围的自动连锁追击能力。
...
### 完整的Tier3 与 Tier4，由于到达这两个层级之后可能性太多，codex额度和人工审核时间都很难完成，所以为了保证稳定性，在这两层做了伪随机，根据Tier2的每种结果安排一种进化路径表现。
